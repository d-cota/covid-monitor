...
class RootNet(nn.Module):
    ...
    def forward(self, x, k_value):
        ...
        coord_x = hm_x * \
                  torch.cuda.comm.broadcast(torch.arange(1, cfg.output_shape[1] + 1).type(torch.cuda.FloatTensor),
                                            devices=[hm_x.device.index])[0]
        coord_y = hm_y * \
                  torch.cuda.comm.broadcast(torch.arange(1, cfg.output_shape[0] + 1).type(torch.cuda.FloatTensor),
